# **Тема №8 Приложение за работа с електронни таблици**
...
2. Всеки ред във файла съдържа данни, разделени с разделител \- запетая, точка и запетая или табулация. Тези данни се интерпретират като стойности в клетките на реда.  
3. Всеки ред в таблицата може да съдържа различен брой клетки, затова и всеки ред във файла може да съдържа различен брой елементи, разделени със съответния разделител.  
4. Празен ред във файла представя празен ред в таблицата (т.е. ред, в който всички клетки са празни).  
5. Между два разделителя във файла може да няма никакви данни. По този начин се представя празна клетка.  
6. Между данните и разделителите може да има произволен брой празни символи (whitespace), които обаче не съвпадат с разделителя.
...
**Символен низ (стринг)** – поредица от произволни символи, оградени в кавички. Подобно на низовете в C++, ако в даден низ трябва да бъде включен символът за кавичка, то той трябва да бъде представен като \\". За да бъде включена наклонена черта, тя трябва да бъде представена като \\\\.  
Например: "Hello world\!"; "C:\\\\temp\\\\"; "\\"This is a quotation\\"".

**Дата** – последователност, описваща **валидна** дата във формат DD-MM-YYYY или YYYY-MM-DD.
...
* Ако липсва разделител, трябва да се изведе на кой ред и след кой символ липсва той.  
...
3. Ако във формула участва дата, то тя се представя като число \- броя дни изминали от 01-01-1970. Това може да е отрицателно число.  
4. Ако в дадена формула участва празна клетка, тя се конвертира до нула. Това важи и за клетки, чиито координати надхвърлят размерите на таблицата.  
5. Ако в дадена формула има грешка (например делене на нула), приложението не трябва да прекъсва своето изпълнение. Вместо това, когато таблицата се извежда на стандартния изход, в съответната клетка се извежда ERROR, вместо получен резултат.
